...
import { MessageSquare, PackageSearch, Send, Users } from "lucide-react";
...
export default async function BuyerDashboardPage() {
  ...
  const [rfqCount, unreadMessages, recentRfqs] = await Promise.all([
    ...
    <div className="space-y-6 pb-16 md:pb-0">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Overview of your RFQs and conversations.</p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard icon={<MessageSquare size={18} />} label="Unread Messages" value={unreadMessages} />
        <StatCard icon={<Users size={18} />} label="Saved Suppliers" value={savedSuppliers} />
        <StatCard icon={<PackageSearch size={18} />} label="Active Orders" value={activeOrders} />
      </div>
            ...
            {recentRfqs.map((r) => (
              ...
              >
                <div className="min-w-0">
                  ...
                  <div className="mt-0.5 text-xs text-muted-foreground">{new Date(r.createdAt).toLocaleDateString()}</div>
                </div>
